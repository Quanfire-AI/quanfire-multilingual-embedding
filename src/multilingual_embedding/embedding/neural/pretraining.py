...
class MaskedLanguageModelTrainer:
    ...
    def train(self, documents: Sequence[str]) -> PretrainingReport:
        """
        ...
        ------
        ValidationError
            If there are no documents to train on.
        """
        ...
        generator = torch.Generator(device=self._encoder.device).manual_seed(config.seed)
        ...
        for epoch in range(config.epochs):
            ...
            for batch in self._batches(documents, generator):
    ...
    def _batches(
        ...
        """
        Yield shuffled batches over the corpus.

        No de-duplication and no minimum size: MLM has no in-batch
        negatives, so a repeated or lone document is a perfectly good
        training example — every batch teaches the same per-token task.
        """

        order = torch.randperm(
            len(documents),
            generator=self._shuffle_generator(generator),
        ).tolist()

        batch_size = self._config.batch_size

        for start in range(0, len(order), batch_size):
            yield [documents[index] for index in order[start : start + batch_size]]

    @staticmethod
    def _shuffle_generator(generator: torch.Generator) -> torch.Generator:
        """
        A CPU generator for shuffling, derived from the run's generator.

        ``torch.randperm`` runs on the CPU, so it needs a CPU generator
        even when the run's masking generator lives on the GPU. Seeding the
        shuffle from the run generator keeps the whole run reproducible from
        the single configured seed.
        """

        if generator.device.type == "cpu":
            return generator

        # The seed draw has to land on the run generator's own device, or
        # the tensor and the generator disagree and torch refuses.
        drawn = torch.randint(
            0,
            2**31 - 1,
            (1,),
        ...
        return torch.Generator().manual_seed(int(drawn.item()))
