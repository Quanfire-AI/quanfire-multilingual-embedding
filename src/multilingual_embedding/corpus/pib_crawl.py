...
def parse_siblings(html):
    m = re.search(r'class="ReleaseLang">(.*?)</div>', html, re.S)
    out = {}
    if m:
...
def parse_release(html):
    return {
        "title": by_id(html, "Titleh2", "h2"),
        "body": parse_body(html),
        "ministry": by_id(html, "MinistryName", "div"),
        "subtitle": by_id(html, "ltrSubtitle", "span"),
...
def main():
    ...
    today = date.today()
    for i in range(DAYS):
        if _fetches >= MAX_FETCHES:
            print("[cap] fetch cap reached", flush=True)
            break
        d = today - timedelta(days=i)
        try:
            anchors = anchors_for(d)
        except Exception as e:
            print(f"[{d}] anchor-list ERR {type(e).__name__}", flush=True)
            continue
        ...
        for ap in new:
            ...
            try:
                ...
                pages = {ap: html}
                for sp in sibs:
                    if sp in seen or sp in pages:
                        continue
                    pages[sp] = get(REL % sp)
                    time.sleep(DELAY)
                label = dict(sibs)
                for p, h in pages.items():
                    for k, v in parse_siblings(h).items():
